...
.PHONY: rc
rc:				## Lunch all dependent services with their rc codebase (param: PROFILE=<profile-name>)
	@docker inspect --type=image instill/tritonserver:${TRITON_SERVER_VERSION} >/dev/null 2>&1 || printf "\033[1;33mINFO:\033[0m This may take a while due to the enormous size of the Triton server image, but the image pulling process should be just a one-time effort.\n" && sleep 5
	@COMPOSE_PROFILES=$(PROFILE) EDITION=rc docker compose -f docker-compose.yml -f docker-compose.edition.yml up -d --quiet-pull
	@COMPOSE_PROFILES=$(PROFILE) EDITION=rc docker compose -f docker-compose.yml -f docker-compose.edition.yml rm -f

.PHONY: latest
latest:			## Lunch all dependent services with their latest codebase (param: PROFILE=<profile-name>)
	@docker inspect --type=image instill/tritonserver:${TRITON_SERVER_VERSION} >/dev/null 2>&1 || printf "\033[1;33mINFO:\033[0m This may take a while due to the enormous size of the Triton server image, but the image pulling process should be just a one-time effort.\n" && sleep 5
	@COMPOSE_PROFILES=$(PROFILE) EDITION=latest docker compose -f docker-compose.yml -f docker-compose.edition.yml up -d --quiet-pull
	@COMPOSE_PROFILES=$(PROFILE) EDITION=latest docker compose -f docker-compose.yml -f docker-compose.edition.yml rm -f
...
.PHONY: build
build:							## Build images for all VDP components (param: EDITION={latest,rc})
	@docker build --progress plain \
		...
		--target $(EDITION) \
		-t instill/vdp-build:$(EDITION) .
	@docker run -it --rm \
		-v /var/run/docker.sock:/var/run/docker.sock \
		-v ${PWD}/.env:/vdp/.env \
		-v ${PWD}/docker-compose.build.yml:/vdp/docker-compose.build.yml \
		--name vdp-build \
		instill/vdp-build:$(EDITION) /bin/bash -c " \
			EDITION=$(EDITION) docker compose -f docker-compose.build.yml build --progress plain \
		"
...
integration-test:			## Run integration test for VDP (param: EDITION={latest,rc})
	@make build $(EDITION)
	@COMPOSE_PROFILES=all EDITION=$(EDITION) ITMODE=true CONSOLE_BASE_URL_HOST=console CONSOLE_BASE_API_GATEWAY_URL_HOST=api-gateway \
		docker compose -f docker-compose.yml -f docker-compose.edition.yml up -d --quiet-pull
	@COMPOSE_PROFILES=all docker compose -f docker-compose.yml -f docker-compose.edition.yml rm -f
	@docker run -it --rm \
		--network instill-network \
		--name backend-integration-test instill/vdp-build:$(EDITION) /bin/bash -c " \
